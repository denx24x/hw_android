...
class MyService : Service() {
    ...
    private fun getMessagesFromCache(){
        try{
            ...
            if(file.exists()){
                ...
                data =  JSONArray(input.readBytes().decodeToString())
                if (data.length() > 0)
                lastKnownId = (data[data.length() - 1] as JSONObject).getInt("id")
                Log.i(TAG, lastKnownId.toString())
    ...
    private fun handlePull(intent: Intent){
        ...
        try {
            ...
            Log.i(TAG, "handlePull error ${e.message}")
        }
        try {
    ...
    private fun moveQueue(){
        ...
        Thread {
            try {
                ...
                Log.i(TAG, "request error - ${e.message.toString()}")
            }finally {
                    mHandler.post{
                        buffer.remove()
                        moveQueue() // notify to start new task
                    }
            }
        }.start()
    }
